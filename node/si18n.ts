...
class Si18n {
  ...
  #noop() {} // Empty function to avoid undefined parameters.
  #isInitialized = false;
  #options = {
    lang: "",
    fallbackLang: "",
  ...
  init(_options = {}) {
    ...
    if (_options.fallbackLang) {
      if (typeof _options.fallbackLang !== "string") {
        throw new Error(`The fallbackLang option must be a string.`);
      }
      this.#options.fallbackLang = _options.fallbackLang;
    } else {
      if (_options.lang) this.#options.fallbackLang = _options.lang;
      else throw new Error(`The lang option is required when no fallbackLang option is set.`);
    }

    if (_options.locales) {
      if (!autoLoad && typeof _options.locales !== "object") {
        throw new Error(`The locales option must be an object.`);
      }
    ...
    const langInURL = process.env.LANG;
    const savedLag = process.env.SAVED_LANG;
    ...
    if (langInURL !== null && isAvailableLocale(langInURL)) {
      ...
      // Use the default language of the system, if it's available.
      const systemLang = process.env.LANG?.substring(0, 2);
      this.#options.lang = isAvailableLocale(systemLang) ?
        systemLang : this.#options.fallbackLang;
    }
    ...
    if (autoLoad) {
      ...
      if (!hasLocalesList || (hasLocalesList && _options.availableLocales.length === 0)) {
        throw new Error(`The availableLocales option is required when using the path option.`);
      }
  ...
  setLocale(lang) {
    ...
    if (saveLang) process.env[saveAs] = lang;
    if (reloadPage) process.exit(0);
  ...
  /**
   * Translates the content by setting/replacing values to/of the elements
  ...
  #translate() {
    ...
    const htmlTags = []; // Placeholder for elements with data-si18n attribute
